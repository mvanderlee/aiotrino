...
async def wait_for_trino_workers(host, port, timeout=180):
...
async def wait_for_trino_coordinator(stream, timeout=180):
...
async def start_trino_and_wait(image_tag=None):
    ...
    if host:
        ...
        container_id, proc, host, port = await start_local_trino_server(
            image_tag
        )
...
async def run_trino():
    ...
    container_id, proc, host, port = await start_trino_and_wait(image_tag)
