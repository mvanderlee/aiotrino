...
async def trino_connection(request, run_trino):
    host, port = run_trino
...
async def test_boolean(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_tinyint(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_smallint(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_int(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_bigint(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_real(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_double(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_decimal(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_varchar(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_char(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_varbinary(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_varbinary_failure(trino_connection):
    await SqlExpectFailureTest(trino_connection) \
        .execute("CAST(42 AS VARBINARY)")


@pytest.mark.asyncio(loop_scope="session")
async def test_json(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_date(trino_connection):
    await (
...
@pytest.mark.asyncio(loop_scope="session")
async def test_time(trino_connection):
    await (
        SqlTest(trino_connection)
        .add_field(
...
@pytest.mark.asyncio(loop_scope="session")
async def test_time_with_timezone(trino_connection, tz_str: str):
...
@pytest.mark.asyncio(loop_scope="session")
async def test_timestamp(trino_connection):
    await (
        SqlTest(trino_connection)
        .add_field(
...
@pytest.mark.asyncio(loop_scope="session")
async def test_timestamp_with_timezone(trino_connection, tz_str):
    tz = create_timezone(tz_str)
    await (
        SqlTest(trino_connection)
        .add_field(
...
@pytest.mark.asyncio(loop_scope="session")
async def test_timestamp_with_timezone_dst(trino_connection):
...
@pytest.mark.asyncio(loop_scope="session")
async def test_interval_year_to_month(trino_connection):
    await (
        SqlTest(trino_connection)
        .add_field(
...
async def test_interval_day_to_second(trino_connection):
    ...
    await SqlExpectFailureTest(trino_connection).execute("INTERVAL '1000000000' DAY")
    await SqlExpectFailureTest(trino_connection).execute("INTERVAL '-999999999 00:00:00.001' DAY TO SECOND")


@pytest.mark.asyncio(loop_scope="session")
async def test_array(trino_connection):
    # primitive types
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_map(trino_connection):
    # primitive types
    await (
        SqlTest(trino_connection)
...
@pytest.mark.asyncio(loop_scope="session")
async def test_row(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_ipaddress(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_uuid(trino_connection):
    await SqlTest(trino_connection) \
...
@pytest.mark.asyncio(loop_scope="session")
async def test_digest(trino_connection):
    await SqlTest(trino_connection) \
...
class SqlTest:
    def __init__(self, trino_connection: aiotrino.dbapi.Connection):
        self.conn = trino_connection
        self.sql_args = []
    ...
    async def execute(self):
        ...
        cur = await self.conn.cursor(legacy_primitive_types=False)
        await cur.execute(sql)
        actual_results = await cur.fetchall()
